...
class CocktailRepositoryTest {
    ...
    private val mockResponse = CocktailResponse(mockCocktails)
    ...
    fun `get remote cocktails by first letter falls back to local when remote fails`() {
        runBlocking {
            ...
            assertEquals(mockCocktails, result.drinks)
    ...
    fun ` get cocktails by first letter returns data from local source`() {
        runBlocking {
            `when`(localDataSource.getCocktailByLetter("M")).thenReturn(mockResponse)
